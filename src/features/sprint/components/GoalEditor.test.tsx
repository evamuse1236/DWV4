...
describe("GoalEditor", () => {
  // --- Happy Path Tests ---
  // Increase timeout to 15s - this test walks through 6 wizard steps with animations
  it("walks through SMART steps and calls onSave", async () => {
    // Use short strings - button only needs non-empty input to be enabled
    // This dramatically reduces typing time (user.type simulates each keypress)
    ...
    // Step 1: Title - use short string
    await user.type(
      screen.getByPlaceholderText(/learn multiplication tables/i),
      "Goal"
    );
    // Remove waitFor - after user.type completes, React state is updated
  ...
  }, 15000); // 15 second timeout for multi-step wizard
